...
void loop()
{
    ...
    // If LEDs should be off (payment not approved, rejected, or session ended)
    else if (!ledFlashingEnabled && !dispensing)
    {
...
void parseASCIIHexString(char* str, uint8_t len)
{
    ...
    if (mdbRxIndex > 0)
    {
        ...
        {
            if (mdbRxBuffer[i] < 0x10) Serial.print("0");
...
void printStateName(MDBState state)
{
    switch (state)
    {
        ...
        case STATE_WAIT_ENABLE_ACK: Serial.print("WAIT_ENABLE_ACK"); break;
        case STATE_WAIT_CARD_TAP: Serial.print("WAIT_CARD_TAP"); break;
        case STATE_WAIT_APPROVAL: Serial.print("WAIT_APPROVAL"); break;
        case STATE_WAIT_ITEM: Serial.print("WAIT_ITEM"); break;
        case STATE_VENDING: Serial.print("VENDING"); break;
        case STATE_WAIT_CANCEL_ACK: Serial.print("WAIT_CANCEL_ACK"); break;
        case STATE_ENDING: Serial.print("ENDING"); break;
...
uint8_t getMDBFrameLength(uint8_t cmd)
{
    switch (cmd)
    {
        case 0x00: return 2; // ACK: 10 00 or single byte 00
        case 0x03: return 4; // Card tap: 10 03 03 E8
        case 0x04: return 2; // Timeout: 10 04
        case 0x05: return 4; // Approved: 10 05 00 0A (4 bytes, amount in bytes 2-3)
        case 0x06: return 2; // Rejected: 10 06
        default: return 2;   // Default minimum
    }
}
...
{
    if (mdbState != STATE_WAIT_ITEM) return;
    if (itemNumber < 1 || itemNumber > 5) return;
    if (buttonIndex >= NUM_BUTTONS) return;
